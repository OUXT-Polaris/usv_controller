...
: Node("usv_controller_node", options),
  ...
  left_thruster_publisher_(
    io_, parameters_.thrusters.left.ip, parameters_.thrusters.left.port, 2000),
  right_thruster_publisher_(
    io_, parameters_.thrusters.right.ip, parameters_.thrusters.right.port, 2001),
...
void UsvControllerComponent::controlFunction()
{
  ...
  const auto send_command = [this](const double left_thrust, const double right_thrust) {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("HOGE"), "Thrust : " << left_thrust << "," << right_thrust);
